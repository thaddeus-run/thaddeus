...
Most local moon tasks (formatters, cleaners) are configured with
`runInCI: 'always'` so they keep working in CI-marked shells like agent
harnesses. Tasks connected to the build graph (dev servers, prod serves) stay
CI-skipped — run those with a `CI=` prefix, e.g. `CI= moonx docs:dev`.
...
default. `.agents/ignore/` is gitignored. Do not put source files, tests, or
committed documentation there.
...
`moonx <project>:typecheck` and `moonx <project>:test`. For docs-only or
AGENTS/skill-only changes, formatting and linting are sufficient unless the edit
touches executable code or package config.
...
- Write comments for readers new to the codepath.
